    ...
    <v-layout wrap>
      ...
      <v-flex md12>
        ...
        <material-card
          ...
          text="Click row to see details."
        >
          <v-btn @click="addSI()" color="green">Add SI</v-btn>
          <v-card-text>
            <v-data-table :headers="siHeaders" :items="SI" hide-actions>
              <template slot="headerCell" slot-scope="{ header }">
                <span
...
export default {
  ...
  methods: {
    ...
    //   this.$router.push({ path: "/pi/edit", query: { id: idPI } });
    // },
    SIDetail(SI) {
      this.$router.push({ path: "/manager/si/detail", query: { id: SI.id } });
    },
    addSI() {
      this.$router.push({ path: "/manager/si/add", query: { id: this.pi } });
    }
  }
};
</script>