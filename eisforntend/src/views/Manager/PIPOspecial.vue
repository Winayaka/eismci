                ...
                <template v-slot:items="props">
                    ...
                    <td class="text-xs-right">
                        <div :key="i" v-for="(itemss, i) in props.item.pipoSpecial2">
                            <p :key="j" v-for="(items, j) in props.item.pipoSpecial2[i].pipoSpecial3">
                                {{ items.poNum }}
                            </p>   
                            <v-divider v-if="i + 1 < itemss.length" :key="`divider-${i}`"></v-divider> 
                        </div>    
                    ...
                    <td class="text-xs-right">
                        <div :key="i" v-for="(itemss, i) in props.item.pipoSpecial2">
                            <div :key="j" v-for="(items, j) in props.item.pipoSpecial2[i].pipoSpecial3">
                                <p :key="k" v-for="(item, k) in props.item.pipoSpecial2[i].pipoSpecial3[j].pipoSpecial4">
                                    {{ item.noSI}} 
                                </p>
                            </div>   
                            <v-divider v-if="i + 1 < itemss.length" :key="`divider-${i}`"></v-divider> 
...
export default {
    ...
    mounted() {
        Axios.get("http://mciexport.herokuapp.com/api/special")
        .then(response => {
