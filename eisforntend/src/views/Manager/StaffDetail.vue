      ...
      <v-flex xs12 md4>
        <material-card class="v-card-profile" title="Staff Details" width="600">
          <v-card-text class="text-xs-left">
            <v-flex xs12 md4>
              <v-text-field label="Name" :value="konten.name" v-model="konten.name" disabled/>
            </v-flex>
            <v-flex xs12 md4>
              <v-text-field label="E-Mail" :value="konten.email" v-model="konten.email" disabled/>
            </v-flex>
            <v-flex xs12 md4>
              <v-text-field
                label="Phone Number"
            ...
            <v-flex xs12 md4>
              <div v-if="konten.gender">
                <v-text-field label="Gender" value="Male" disabled/>
              </div>
              <div v-else>
                <v-text-field label="Gender" value="Female" disabled/>
              </div>
            </v-flex>
            <v-flex xs12 md4>
              <v-text-field
    ...
    <v-layout justify-start wrap>
      <v-flex xs12 md4>
        <material-card class="v-card-profile" title="User Details" width="400">
          ...
          <v-flex xs12 md4>
            <v-text-field label="Role" :value="konten.role" v-model="konten.role" disabled/>
          </v-flex>
          <v-flex xs12 md4>
            <div v-if="konten.activeStatus">
              <v-text-field label="Active Status" value="Active" disabled/>
            </div>
            <div v-else>
              <v-text-field label="Active Status" value="Not Active" disabled/>
            </div>
          </v-flex>
  ...
  methods: {
    ...
    toEdit() {
      this.$router.push({
        ...
        query: { staffName: this.konten.name }
      });
    }
  }
};
