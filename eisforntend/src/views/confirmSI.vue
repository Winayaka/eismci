    ...
    >
      ...
      >
        ...
        -->
          ...
          <v-text-field 
            label="Invoice"
            :value="konten.invoice"
            v-model="konten.invoice"
          />
          <v-text-field 
            label="Vessel"
          ...
          <v-text-field 
            label="SI Document"
            :value="konten.siDocument"
            v-model="konten.siDocument"
          />

           <v-text-field 
            label="Booking Confirmation"
            :value="konten.bookingConfirmation"
            v-model="konten.bookingConfirmation"
            disabled
          />
                ...
                <v-card-text>
                  <v-container grid-list-md>
                    <v-layout wrap>
                      ...
                      <v-flex xs12 sm6 md4>
                        <v-text-field type="number" v-model="editedItem.quantity" label="Quantity" :rules="rules.required"></v-text-field>
                      </v-flex>
                    </v-layout>
                  </v-container>
                </v-card-text>
...
export default {
    data: () => ({
       ...
       siDocument : ['Issued','On Process'],
       bookingConfirmation : ['Received','On Process'],
       staff: [],
       editedItem: {
         pipoid: '',
         productName: '',
    ...
    beforeCreate: function () {
        ...
        //rolenya sokap?
        if (!this.$session.get("role") === "manager" || "StaffExport") {
          this.$router.push('/')
          console.log("bukan staff")
        }
    ...
    mounted () {
        ...
        .then(response => { this.staff = response.data.result })
        .catch(function error(params) {})
        Axios.get('http://mciexport.herokuapp.com/api/getAllPIPOTrans' )
        .then(response => { this.pipotrans = response.data.result })
        .catch(function error(params) {})
    ...
    methods: {
      ...
      deleteItem (item) {
        ...
        confirm('Are you sure you want to delete this item?') && this.konten.produkDetail.splice(index, 1)
      },
      close () {
        this.dialog = false
        setTimeout(() => {
      ...
      save () {
        if (this.editedIndex > -1) {
          Object.assign(this.konten.produkDetail[this.editedIndex], this.editedItem)
      ...
      submitSI(SI){
        var idSi = this.$route.query.id;
      console.log(SI.invoice)
      var url = 'http://mciexport.herokuapp.com/api/si/update?id='+ SI.shipmentId
      console.log(url)
      Axios.post(url, 
        {
          ...
          invoice: SI.invoice,
          bookingConfirmation : SI.bookingConfirmation,
          siDocument : SI.siDocument
        },
