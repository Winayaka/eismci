      ...
      >
        ...
        <p class="style font-italic">Please fill out customer information below.</p>
          ...
          >
            <v-text-field
              label="Title"
              v-model="konten.title"
              required
              />
            <v-text-field
              label="Name"
              v-model="konten.custName"
              required
              />       
            <v-text-field
              label="First Name"
...
export default {
  ...
  mounted () {
    ...
    var url = 'http://mciexport.herokuapp.com/api/getCustomer?custName='+namaCustomer
    console.log(url)
    axios.get(url)
      .then(response => { 
  ...
  methods: {
      ...
      activateDeactivate(customer){
        ...
        axios.post('http://localhost:8099/api/changeCustomerStatus?id='+ customer.id, {
      })
      ...
      editCustomer(customer){ 
       ...
       if (customer.phoneNumber == ""|| !/^[0-9]\d*$/.test(customer.phoneNumber)){
         isOk = false
       ...
       if (customer.addressNumber == "" || !/^[a-zA-Z0-9]+$/.test(customer.addressNumber)){
         isOk = false
         console.log("no alamat kosong")
       }
       if (customer.postalCode == "" || !/^[0-9]\d*$/.test(customer.postalCode)){
         isOk = false
       ...
       if (customer.description == "" || !/^[ a-zA-Z0-9]+$/.test(customer.description)){
         isOk = false
       ...
       if(isOk){
          ...
          axios({
            method: 'post',
            url: 'http://mciexport.herokuapp.com/api/editCustomer?id=' + customer.id,
            data: {
